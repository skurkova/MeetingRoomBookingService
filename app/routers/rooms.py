...
@router.get("/", response_model=List[RoomAvailableResponse], status_code=status.HTTP_200_OK)
async def get_rooms(target_date: date | None = Query(None, description="Фильтр слотов по дате (YYYY-MM-DD)"),
                    db: AsyncSession = Depends(get_db)):
    """Получить список всех комнат с их временными слотами"""

    booking_date = target_date or date.today()
    bookings_for_date_result = await db.execute(select(Booking.room_slot_id).where(
    ...
    for room in rooms:
        ...
        for room_slot in room.room_slots:
            room_slots.append(RoomSlotAvailableResponse(
                id=room_slot.id,
                room_id=room.id,
                time_slot=room_slot.time_slot,
                is_available=room_slot.id not in booking_room_slots_ids
            ))
        rooms_is_available_slots.append(RoomAvailableResponse(
            id=room.id,
            name=room.name,
...
@router.get("/{room_id}", response_model=RoomAvailableResponse, status_code=status.HTTP_200_OK)
async def get_room_id(room_id: int,
                      target_date: date | None = Query(None, description="Фильтр слотов по дате (YYYY-MM-DD)"),
                      db: AsyncSession = Depends(get_db)):
    """Получить комнату с временными слотами"""

    booking_date = target_date or date.today()
    bookings_room_for_date_result = await db.execute(select(Booking.room_slot_id).join(Booking.room_slot).where(
        Booking.booking_date == booking_date,
